...
class OrderView(BaseTemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        data = dict(
            ...
            WMI_PAYMENT_AMOUNT=tariff.cost,
            WMI_CURRENCY_ID='643',
            WMI_PAYMENT_NO=order.id,
            WMI_DESCRIPTION='Оплата счета №{}'.format(order.id),
            WMI_SUCCESS_URL=reverse('payment_success'),
            WMI_FAIL_URL=reverse('payment_fail'),
            WMI_EXPIRED_DATE=expired_date.strftime('%Y-%m-%d %H:%M:%S'),
